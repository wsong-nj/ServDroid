...
public class CGExporter {
    ...
    public void linkNodeByID(WTGEdge edge) {
    	String srcNode=edge.getSrcNode().toString();
		String tarNode=edge.getTgtNode().toString();
        Node sourceNode = this.getNodeByID(srcNode);
        Node targetNode = this.getNodeByID(tarNode);
        if (sourceNode.equals(targetNode)) {
            return;
        }
        if (!sourceNode.hasEdgeTo(tarNode)) {
            String edgeID = srcNode + "-->"+edge.getEventSeqToString()+"-->" + tarNode;
